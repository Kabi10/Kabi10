...
const COMMODITY_MAP = {
  ...
  "potatoes (imported)": {
    type: "POTATO_IMPORTED",
    en: "Potatoes (Imported)",
...
async function fetchHDXPrices() {
  const url = new URL(
    `${HDX_HAPI_BASE}/food-security-nutrition-poverty/food-prices-market-monitor`,
  );
  ...
  if (!response.ok) {
    throw new Error(
      `HDX HAPI responded with ${response.status}: ${response.statusText}`,
    );
  }
...
function transformPrices(hdxRecords) {
  ...
  for (const [key, { records, commodity, market }] of groupedByKey) {
    ...
    const previousPrice = previous
      ? parseFloat(previous.price) || currentPrice
      : currentPrice;
...
async function upsertPrices(prices) {
  ...
  if (deleteError) {
    logger.error("Failed to delete old HDX prices", {
  ...
  if (insertError) {
    logger.error("Failed to batch insert market prices", {
