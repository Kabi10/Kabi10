 ...
 * Supports Mobitel mSpace (recommended), Dialog Ideamart, and Twilio fallback
 *
 ...
 * - mobitel: Send via Mobitel mSpace API (RECOMMENDED - primary Sri Lankan provider)
 * - dialog: Send via Dialog Ideamart API (unresponsive - not recommended)
 * - twilio: Send via Twilio API (international fallback)
 *
 * Production setup: Set SMS_MODE=mobitel with MOBITEL_USERNAME and MOBITEL_PASSWORD
 */
class SMSService {
  constructor() {
  ...
  async sendViaDialog(phoneNumber, message) {
    const apiUrl =
      process.env.DIALOG_API_URL || "https://api.dialog.lk/sms/send";
    ...
    throw new Error(
      `Dialog API error: ${response.data.message || "Unknown error"}`,
    );
  }
  ...
  async sendViaMobitel(phoneNumber, message) {
    ...
    throw new Error(
      `Mobitel API error: ${response.data.message || "Unknown error"}`,
    );
  }
  ...
  getCostEstimate(mode = this.mode) {
    const costs = {
      mock: 0.0, // Free - no SMS sent
      dialog: 0.5, // ~0.50 LKR per SMS
      mobitel: 0.45, // ~0.45 LKR per SMS
      twilio: 2.5, // ~2.50 LKR per SMS (international rates)
    };

    return costs[mode] || 1.0;
  }
    ...
    return {
      ...
      available: {
        ...
        dialog: {
          ...
          configured: !!(
            process.env.DIALOG_API_KEY && process.env.DIALOG_API_SECRET
          ),
          costPerSMS: 0.5,
          coverage: "Sri Lanka",
          recommended: true,
        ...
        mobitel: {
          name: "Mobitel mSpace",
          configured: !!(
            process.env.MOBITEL_USERNAME && process.env.MOBITEL_PASSWORD
          ),
          costPerSMS: 0.45,
          coverage: "Sri Lanka",
          recommended: true,
