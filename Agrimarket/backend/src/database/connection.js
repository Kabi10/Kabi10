...
const mockDb = {
  async query(text, params = []) {
    ...
    // Handle OTP queries
    if (
      text.includes("FROM otp_verifications") &&
      text.includes("created_at > NOW()")
    ) {
    ...
    if (
      ...
      const phoneNumber = params.find(
        (p) => typeof p === "string" && p.startsWith("+94"),
      );
    ...
    if (text.includes("COUNT(*)") && text.includes("listings")) {
      return {
        rows: [{ total: String(mockStorage.listings.length) }],
        rowCount: 1,
      };
    }
    ...
    if (text.includes("COUNT(*)") && text.includes("transactions")) {
      return {
        rows: [{ total: String(mockStorage.transactions.length) }],
        rowCount: 1,
      };
    }
