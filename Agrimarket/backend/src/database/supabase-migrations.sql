...
-- Mirrors the original Node.js backend schema exactly
...
-- Local operations table (mirrors Android LocalOp entity exactly)
...
-- Indexes for performance (matching original backend)
...
ALTER TABLE local_ops ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Farmers can manage own listings" ON listings
...
-- Local ops policies
CREATE POLICY "Users can manage own local ops" ON local_ops
    FOR ALL USING (auth.uid()::text = user_id::text);

CREATE POLICY "Service role can manage local ops" ON local_ops
    FOR ALL USING (auth.role() = 'service_role');
