...
/**
 * POST /api/v1/sync
 * Alias for /operations — matches Android SyncApiService.syncData()
 * Accepts { operations, lastSyncTimestamp } and returns the same envelope
 * as /operations.
 */
router.post("/", async (req, res) => {
  try {
    const { operations, lastSyncTimestamp, lastSyncAt } = req.body;

    // Accept either field name so both callers work
    const effectiveLastSyncAt = lastSyncAt || lastSyncTimestamp;
