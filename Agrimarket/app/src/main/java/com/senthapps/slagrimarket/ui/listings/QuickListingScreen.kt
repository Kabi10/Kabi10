...
fun QuickListingScreen(
    ...
    val cameraPermissionLauncher = rememberLauncherForActivityResult(
        ...
        if (granted) {
            try {
                ...
                // Camera unavailable, stay on photo step
            }
        } else {
            android.widget.Toast.makeText(
                context,
                context.getString(R.string.quick_listing_camera_denied),
    ...
    fun launchCamera() {
        try {
            ...
            android.widget.Toast.makeText(
                context,
                context.getString(R.string.quick_listing_camera_unavailable),
                android.widget.Toast.LENGTH_LONG
            ).show()
        }
    }
...
private fun PhotoStep(
    ...
    Column(
        ...
        if (photoUri != null) {
            ...
            AsyncImage(
                model = photoUri,
                contentDescription = stringResource(R.string.quick_listing_photo_desc),
            ...
            Text(
                text = when (language) {
                    AppLanguage.SINHALA -> "නිෂ්පාදනයේ ඡායාරූපයක්\nඑකතු කරන්න"
                    AppLanguage.TAMIL -> "பொருளின் புகைப்படம்\nசேர்க்கவும்"
            ...
            SecondaryButton(
                text = when (language) {
                    AppLanguage.SINHALA -> "ඡායාරූපයකින් තොරව ඉදිරියට"
                    AppLanguage.TAMIL -> "புகைப்படமின்றி தொடரவும்"
                    AppLanguage.ENGLISH -> "SKIP PHOTO"
                },
                onClick = onNext
            )
        }
...
private fun DetailsStep(
    ...
    fun tryGpsAutoFill() {
        try {
            ...
            // GPS unavailable or permission denied at runtime
        }
    }
    ...
    Column(
        ...
        val yieldTips = mapOf(
                "tomato" to stringResource(R.string.quick_listing_tip_tomato),
                "red onion" to stringResource(R.string.quick_listing_tip_red_onion),
                "chili" to stringResource(R.string.quick_listing_tip_chili),
                "brinjal" to stringResource(R.string.quick_listing_tip_brinjal),
                "okra" to stringResource(R.string.quick_listing_tip_okra)
            )
