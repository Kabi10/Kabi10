...
fun IndustrialSettingsScreen(
    ...
    Column(
        ...
        SectionLabel(
            text = when (currentLanguage) {
                AppLanguage.SINHALA -> "නීතිමය"
                AppLanguage.TAMIL -> "சட்டம்"
        ...
        Row(
            modifier = Modifier
                ...
                .height(96.dp)
                .background(HumanIndustrial.Rice)
                .industrialClickable { onNavigateToPrivacy() }
                .padding(horizontal = Spacing.lg.dp),
            verticalAlignment = Alignment.CenterVertically
        ) {
            Text(
                text = when (currentLanguage) {
                    AppLanguage.SINHALA -> "රහස්‍යතා ප්‍රතිපත්තිය"
        ...
        Row(
            modifier = Modifier
                ...
                .industrialClickable { onNavigateToTerms() }
                .padding(horizontal = Spacing.lg.dp),
            verticalAlignment = Alignment.CenterVertically
        ) {
            Text(
                text = when (currentLanguage) {
                    AppLanguage.SINHALA -> "සේවා කොන්දේසි"
                    AppLanguage.TAMIL -> "சேவை விதிமுறைகள்"
