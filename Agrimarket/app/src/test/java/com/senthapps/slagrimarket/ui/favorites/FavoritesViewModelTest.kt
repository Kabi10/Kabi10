...
class FavoritesViewModelTest {
    ...
    @Test
    fun `removeFavorite should set error on repository exception`() = runTest {
        coEvery { authRepository.getCurrentUser() } returns mockUser
        coEvery { favoriteRepository.toggleFavorite(any(), any()) } throws RuntimeException("Network error")
        viewModel = FavoritesViewModel(favoriteRepository, authRepository)
        ...
        assertNotNull(error)
        assertTrue(error!!.contains("Failed to remove favorite"))
    }

    @Test
    fun `removeFavorite success does not set error state`() = runTest {
        coEvery { authRepository.getCurrentUser() } returns mockUser
        coEvery { favoriteRepository.toggleFavorite("user1", "listing2") } returns Result.success(false)
        viewModel = FavoritesViewModel(favoriteRepository, authRepository)

        viewModel.removeFavorite("listing2")
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `clearAllFavorites when user not authenticated sets user not found error`() = runTest {
        coEvery { authRepository.getCurrentUser() } returns null
        viewModel = FavoritesViewModel(favoriteRepository, authRepository)
