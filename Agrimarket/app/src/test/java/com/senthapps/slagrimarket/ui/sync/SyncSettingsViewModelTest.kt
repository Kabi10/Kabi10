...
class SyncSettingsViewModelTest {
    ...
    @Test
    fun `refreshStatistics includes conflictCount from syncManager stats`() = runTest {
        val stats = SyncStatistics(
            pendingOperations = 0,
        ...
        assertEquals(3, viewModel.uiState.value.conflictCount)
    }

    @Test
    fun `dismissConflicts delegates to syncManager`() = runTest {
        coEvery { syncManager.dismissConflicts() } returns Unit
        viewModel = SyncSettingsViewModel(syncManager)
        testDispatcher.scheduler.advanceUntilIdle()
        ...
        coVerify { syncManager.dismissConflicts() }
    }

    @Test
    fun `dismissConflicts clears conflictCount via syncManager state`() = runTest {
        syncStateFlow.value = SyncState(conflictCount = 2)
        coEvery { syncManager.dismissConflicts() } coAnswers {
            syncStateFlow.value = SyncState(conflictCount = 0)
        }
        viewModel = SyncSettingsViewModel(syncManager)
        testDispatcher.scheduler.advanceUntilIdle()
        ...
        assertEquals(0, viewModel.uiState.value.conflictCount)
    }

    @Test
    fun `conflictCount in uiState updates when syncState emits new conflicts`() = runTest {
        viewModel = SyncSettingsViewModel(syncManager)
        testDispatcher.scheduler.advanceUntilIdle()

        syncStateFlow.value = SyncState(conflictCount = 4)
        testDispatcher.scheduler.advanceUntilIdle()

        assertEquals(4, viewModel.uiState.value.conflictCount)
    }
}
