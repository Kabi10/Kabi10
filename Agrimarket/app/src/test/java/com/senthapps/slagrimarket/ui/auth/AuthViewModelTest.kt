...
class AuthViewModelTest {
    ...
    @Test
    fun `loginWithPassword should set authenticated state on success`() = runTest {
        coEvery { repository.loginWithPassword("+94771234567", "pass123") } returns Result.success(mockUser)
        viewModel = createViewModel()

        viewModel.loginWithPassword("+94771234567", "pass123")
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `loginWithPassword should set error state on failure`() = runTest {
        coEvery { repository.loginWithPassword(any(), any()) } returns Result.failure(Exception("Wrong password"))
        viewModel = createViewModel()

        viewModel.loginWithPassword("+94771234567", "wrong")
        testDispatcher.scheduler.advanceUntilIdle()
        ...
        assertFalse(state.isAuthenticated)
        assertNotNull(state.error)
        assertTrue(state.error!!.contains("Wrong password"))
    }

    @Test
    fun `registerWithPassword should set authenticated state on success`() = runTest {
        coEvery { repository.registerWithPassword("+94771234567", "pass123", "FARMER") } returns Result.success(mockUser)
        viewModel = createViewModel()

        viewModel.registerWithPassword("+94771234567", "pass123", "FARMER")
        testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `registerWithPassword should set error state on failure`() = runTest {
        coEvery { repository.registerWithPassword(any(), any(), any()) } returns Result.failure(Exception("Phone already registered"))
        viewModel = createViewModel()

        viewModel.registerWithPassword("+94771234567", "pass123", "BUYER")
        testDispatcher.scheduler.advanceUntilIdle()
        ...
        assertFalse(state.isAuthenticated)
        assertNotNull(state.error)
        assertTrue(state.error!!.contains("Phone already registered"))
    }

    @Test
    fun `sendOtp with returned otp code should auto-verify and authenticate`() = runTest {
        coEvery { repository.sendOtp("+94771234567") } returns Result.success(
            AuthRepository.OtpResult("otp123", "654321")
        )
        coEvery { repository.verifyOtp("+94771234567", "654321", "otp123") } returns Result.success(mockUser)
        viewModel = createViewModel()
        ...
        assertTrue(state.isAuthenticated)
        assertEquals(mockUser, state.user)
        coVerify { repository.verifyOtp("+94771234567", "654321", "otp123") }
    }
}
