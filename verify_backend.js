...
async function runTests() {
  ...
  try {
    ...
    console.log(`   ✅ Success. Found ${listings.data.count || listings.data.data.length} listings.`);

    // 5. Create Listing (Expect Success or 403)
    console.log("\n5️⃣  Testing CREATE Listing (POST /api/listings/create)...");
    try {
      const newListing = await axios.post(`${BASE_URL}/api/listings/create`, {
        cropType: "tomato",
        quantity: 100,
        unit: "kg",
        pricePerUnit: 250,
      ...
      if (error.response && error.response.status === 403) {
        console.log(`   ⚠️  Received 403 Forbidden (Expected if user is not a FARMER).`);
        console.log(`   ✅ Authorization Logic Working: Buyer cannot create listings.`);
      } else {
        throw error;
      }
    }
