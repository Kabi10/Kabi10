...
async function seed() {
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      await db.query(
        "TRUNCATE TABLE local_ops, transactions, listings, users, otp_verifications, sync_metadata RESTART IDENTITY CASCADE",
      );
    ...
    for (const listing of listings) {
      ...
      logger.info(
        `Created listing: ${listing.cropType} by farmer ${listing.farmerId}`,
      );
    }
    ...
    for (const transaction of transactions) {
      ...
      logger.info(
        `Created transaction: ${result.rows[0].id} (${transaction.status})`,
      );
    }
