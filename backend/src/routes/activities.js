...
function mapActivityToDto(row, language = 'en') {
  return {
    ...
    title: language === 'ta' ? (row.title_tamil || row.title) :
           language === 'si' ? (row.title_sinhala || row.title) :
           row.title,
    description: language === 'ta' ? (row.description_tamil || row.description) :
                 language === 'si' ? (row.description_sinhala || row.description) :
                 row.description,
...
router.get('/',
  ...
  async (req, res) => {
    try {
      ...
      const activities = result.rows.map(row => mapActivityToDto(row, language));
...
router.get('/summary',
  ...
  async (req, res) => {
    try {
      ...
      const language = req.query.language || 'en';
      const recentActivities = recentResult.rows.map(row => mapActivityToDto(row, language));
  ...
  async (req, res) => {
    try {
      ...
      const result = await db.query(`
        ...
        RETURNING *
      `, [
        userId, activityType, title, titleTamil, titleSinhala,
        description, descriptionTamil, descriptionSinhala,
        relatedEntityType, relatedEntityId,
        priority, isActionable, expiresAt, JSON.stringify(metadata)
      ]);
